...
/// 2. 从数据库读出 work_logs（include_screenshots=false 时排除 screenshot）。
pub fn collect_data(
    _cfg: &Config,
...
fn build_user_prompt(
    ...
    // —— Git 提交（研发证据） ——
    let git_logs: Vec<&storage::WorkLog> = prepared
        .work_logs
        .iter()
        .filter(|l| l.source == "git")
        .collect();
    if !git_logs.is_empty() {
        out.push_str("## Git 提交（代码工作证据）\n");
        for g in &git_logs {
            let _ = writeln!(
                out,
                "- [{}] {}",
    ...
    // —— 其他来源（手动录入等；todo / git 已在上方专节出现，这里排除） ——
    let others: Vec<&storage::WorkLog> = prepared
        .work_logs
        .iter()
        .filter(|l| l.source != "screenshot" && l.source != "todo" && l.source != "git")
        .collect();
    if !others.is_empty() {
        out.push_str("## 其他工作记录\n");
        for o in others {
