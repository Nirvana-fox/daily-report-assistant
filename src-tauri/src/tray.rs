...
pub fn setup(app: &AppHandle) -> tauri::Result<()> {
    ...
    let menu = Menu::with_items(app, &[&show, &todos, &toggle, &llm_mode, &logs, &quit])?;
        ...
        .on_menu_event(|app, ev| match ev.id.as_ref() {
            ...
            "toggle_llm_mode" => {
                ...
                tauri::async_runtime::spawn_blocking(move || {
                    if let Err(e) = crate::commands::toggle_local_llm_impl(&app) {
                        tracing::warn!("托盘切换本地模型失败: {e}");
                    }
                });
            }
            "open_logs" => {
